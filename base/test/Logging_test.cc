...
void bench() {
    duo::Logger::setOutput(dummyOutput);
    duo::Timestamp start(duo::Timestamp::now());
    g_total = 0;

    const int batch = 1000 * 1000;
    const bool kLongLog = false;
    duo::string empty = " ";
    duo::string longStr(3000, 'X');
    longStr += " ";
    ...
    duo::Timestamp end(duo::Timestamp::now());
...
int main() {
    ...
    LOG_WARN << "World";
    LOG_ERROR << "Error";
    LOG_INFO << sizeof(duo::Logger);
    LOG_INFO << sizeof(duo::LogStream);
    ...
    g_logFile.reset(new duo::LogFile("test_log_mt", 500 * 1000 * 1000, true));
    bench();
    g_logFile.reset();
}
